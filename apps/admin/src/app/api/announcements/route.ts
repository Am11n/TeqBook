...
import type { AnnouncementInput, AnnouncementListFilters } from "@/lib/types/announcements";
...
export async function GET(request: Request) {
  ...
  const status = (searchParams.get("status") ?? "all") as AnnouncementListFilters["status"];
  const { data, error } = await listAnnouncements(auth.client, { status });
...
export async function POST(request: Request) {
  ...
  const { data, error } = await createAnnouncement(auth.client, auth.user.id, {
    title,
    body: content,
