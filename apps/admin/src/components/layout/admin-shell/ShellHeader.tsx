...
interface ShellHeaderProps {
  ...
  shellBrand: AdminConsoleMessages["widgets"]["shellBrand"];
}
...
function UserAvatarMenu({
  profile, userEmail, loggingOut, onSignOut, className, shell,
}: Pick<ShellHeaderProps, "profile" | "userEmail" | "loggingOut" | "onSignOut" | "shell"> & { className?: string }) {
  return (
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button className="flex items-center justify-center h-9 w-9 transition-all hover:scale-105">
            <Avatar className="h-9 w-9 border-2 border-card shadow-sm transition-all hover:shadow-md">
              {profile?.avatar_url && <AvatarImage src={profile.avatar_url} alt={shell.profileAvatarAlt} />}
        ...
        <DropdownMenuContent align="end" className="w-56 rounded-xl bg-white/90 backdrop-blur-md border border-neutral-200/70 shadow-lg">
            ...
            <div className="flex flex-col space-y-1">
              <p className="text-sm font-medium leading-none">
                {profile?.first_name && profile?.last_name
                  ? `${profile.first_name} ${profile.last_name}`
                  : userEmail || shell.adminFallbackName}
              </p>
              <p className="text-xs leading-none text-muted-foreground">{shell.superAdminBadge}</p>
            </div>
          </DropdownMenuLabel>
          <DropdownMenuSeparator />
          <DropdownMenuItem asChild>
            <Link href="/profile" className="flex items-center gap-2 cursor-pointer">
              <User className="h-4 w-4" /><span>{shell.myProfile}</span>
            </Link>
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem
          ...
          >
            ...
            <span>{loggingOut ? shell.loggingOut : shell.logOut}</span>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
    ...
    <header className="sticky top-0 z-[50] flex h-[72px] w-full items-center justify-between border-b border-border bg-card/95 backdrop-blur-md shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
      <div className="flex items-center gap-3 pl-6">
        <Button
          ...
          onClick={onOpenMobileNav} aria-label={shell.openNavigationAria}
        >
        ...
        <Link href="/" className="hidden items-center gap-3 transition-all duration-150 hover:scale-105 hover:drop-shadow-[0_2px_8px_rgba(29,78,216,0.15)] md:flex">
          <Image src={FAVICON_PATH} alt={shellBrand.logoAlt} width={32} height={32} className="h-8 w-8 transition-transform duration-150" priority />
          <span className="text-lg font-semibold tracking-tight text-foreground">{shellBrand.shortTitle}</span>
        </Link>
        <Link href="/" className="flex items-center gap-2 transition-opacity hover:opacity-80 md:hidden">
          <Image src={FAVICON_PATH} alt={shellBrand.logoAlt} width={28} height={28} className="h-7 w-7" priority />
          <span className="text-base font-semibold tracking-tight text-foreground">{shellBrand.shortTitle}</span>
        </Link>
      </div>
      ...
      <div className="hidden flex-1 items-center justify-center px-4 md:flex">
        <button
          ...
          aria-label={shell.openCommandPalette}
        ...
        >
          <Search className="h-4 w-4 text-muted-foreground transition-colors group-hover:text-primary" aria-hidden />
          <span className="flex-1 text-sm text-muted-foreground group-hover:text-foreground">
            {shellBrand.searchPlaceholder}
          </span>
      ...
      <div className="flex items-center gap-2 pr-6">
        <LanguageSelector locale={locale} salon={salon} setLocale={setLocale} languageAria={shell.languageAria} />
        <NotificationCenter />
        <UserAvatarMenu profile={profile} userEmail={userEmail} loggingOut={loggingOut} onSignOut={onSignOut} shell={shell} className="hidden sm:flex" />
        <UserAvatarMenu profile={profile} userEmail={userEmail} loggingOut={loggingOut} onSignOut={onSignOut} shell={shell} className="flex sm:hidden" />
      </div>
    </header>
  );
}
