...
export default function GiftCardsPage() {
  ...
  const handleDeactivate = async (cardId: string) => {
    if (!salon?.id) return;
    await deactivate(salon.id, cardId);
    setCards((prev) => prev.map((c) => (c.id === cardId ? { ...c, is_active: false } : c)));
  };
  ...
  return (
    <ErrorBoundary>
      ...
      <div className="rounded-xl border bg-card p-4 shadow-sm">
        {loading ? (
          <p className="text-sm text-muted-foreground py-4">Loading gift cards...</p>
        ) : filtered.length === 0 ? (
          <EmptyState title="No gift cards yet" description="Create your first gift card to get started." />
        ) : (
          <div className="divide-y">
            {filtered.map((card) => (
              <div key={card.id} className="flex items-center justify-between py-3 gap-4">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    >
                      {copiedId === card.id ? <Check className="h-3 w-3 text-green-600" /> : <Copy className="h-3 w-3" />}
                    </button>
                  </div>
                  {card.recipient_name && (
                    <span className="text-xs text-muted-foreground truncate">{card.recipient_name}</span>
                  )}
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  <div className="text-right">
                    <p className="text-sm font-medium">{fmtPrice(card.remaining_value_cents)}</p>
                    {card.remaining_value_cents < card.initial_value_cents && (
                      <p className="text-[10px] text-muted-foreground">of {fmtPrice(card.initial_value_cents)}</p>
                    )}
                  </div>
                  ...
                  {card.is_active && (
                    <Button size="sm" variant="ghost" className="h-7 text-xs text-red-600" onClick={() => handleDeactivate(card.id)}>
                      Deactivate
                    </Button>
                  )}
                </div>
