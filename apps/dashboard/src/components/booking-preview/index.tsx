...
export function BookingPreview({ salonSlug, theme }: BookingPreviewProps) {
  ...
  const langLabels: Record<AppLocale, string> = {
    nb: "Norsk", en: "English", ar: "العربية", so: "Soomaali", ti: "ትግርኛ",
    am: "አማርኛ", tr: "Türkçe", pl: "Polski", vi: "Tiếng Việt", tl: "Tagalog",
    zh: "中文", fa: "فارسی", dar: "دری (Dari)", ur: "اردو", hi: "हिन्दी",
  };
        ...
        <div className="mx-auto flex max-w-xl flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="mt-2 flex items-center gap-2 text-xs sm:mt-0">
            {salon.whatsapp_number && (
              <a href={`https://wa.me/${salon.whatsapp_number.replace(/[^0-9]/g, "")}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-full border border-green-600 bg-green-50 px-3 py-1.5 text-xs font-medium text-green-700 transition hover:bg-green-100">
            ...
            {salon.supported_languages && salon.supported_languages.length > 0 && (
              <DialogSelect
                ...
                onChange={(v) => {
                  ...
                  setLocale(newLocale);
                  if (typeof window !== "undefined" && salon.id) localStorage.setItem(`booking-locale-${salon.id}`, newLocale);
                }}
                options={salon.supported_languages.map((lang) => ({ value: lang, label: langLabels[lang as AppLocale] || lang }))}
              />
            )}
          </div>
        </div>
        ...
        <section className="space-y-4 rounded-2xl border bg-card p-4 shadow-sm">
          ...
          <form onSubmit={handleLoadSlots} className="space-y-4">
            <div className="space-y-2 text-sm">
              <label className="font-medium" htmlFor="preview-service">{t.serviceLabel}</label>
              <DialogSelect value={serviceId} onChange={setServiceId} required placeholder={t.servicePlaceholder} options={[{ value: "", label: t.servicePlaceholder }, ...services.map((s) => ({ value: s.id, label: s.name }))]} />
            </div>
            <div className="space-y-2 text-sm">
              <label className="font-medium" htmlFor="preview-employee">{t.employeeLabel}</label>
              <DialogSelect value={employeeId} onChange={setEmployeeId} required placeholder={t.employeePlaceholder} options={[{ value: "", label: t.employeePlaceholder }, ...employees.map((e) => ({ value: e.id, label: e.full_name }))]} />
            </div>
            <div className="space-y-2 text-sm">
          ...
          <div className="space-y-2 text-sm">
            ...
            <DialogSelect value="" onChange={() => {}} disabled placeholder={t.noSlotsYet} options={[]} />
          </div>
        </section>
        ...
        <section className="space-y-4 rounded-2xl border bg-card p-4 shadow-sm">
          ...
          <form onSubmit={handleSubmitBooking} className="space-y-3">
            <div className="space-y-1 text-sm">
              <label className="font-medium" htmlFor="preview-customer_name">{t.nameLabel}</label>
              <Input id="preview-customer_name" type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} required />
            </div>
            <div className="space-y-1 text-sm">
              <label className="font-medium" htmlFor="preview-customer_email">{t.emailLabel}</label>
              <Input id="preview-customer_email" type="email" value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)} placeholder={t.emailPlaceholder} />
            </div>
            <div className="space-y-1 text-sm">
