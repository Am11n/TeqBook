...
function UserMenu({ profile, userEmail, loggingOut, onSignOut, className }: {
  profile: ShellHeaderProps["profile"];
  userEmail: string | null;
  loggingOut: boolean;
  onSignOut: () => void;
  className?: string;
    ...
    <div suppressHydrationWarning>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button className={`items-center transition-all hover:scale-105 ${className}`}>
  ...
  return (
    <header className="sticky top-0 z-[50] flex h-[72px] w-full items-center justify-between border-b border-border bg-card/95 backdrop-blur-md shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
      <div className="flex items-center gap-3 pl-6">
        ...
        <Link href="/admin" className="hidden items-center gap-3 transition-all duration-150 hover:scale-105 hover:drop-shadow-[0_2px_8px_rgba(29,78,216,0.15)] md:flex">
          <Image src="/Favikon.svg" alt="TeqBook Admin" width={32} height={32} className="h-8 w-8 transition-transform duration-150" priority />
          <span className="text-lg font-semibold tracking-tight text-foreground">TeqBook Admin</span>
        </Link>
        <Link href="/admin" className="flex items-center gap-2 transition-opacity hover:opacity-80 md:hidden">
          <Image src="/Favikon.svg" alt="TeqBook Admin" width={28} height={28} className="h-7 w-7" priority />
      ...
      <div className="hidden flex-1 items-center justify-center px-4 md:flex">
        ...
        >
          ...
          <span className="flex-1 text-sm text-muted-foreground group-hover:text-foreground">Search salons, users, analytics...</span>
          <kbd className="hidden rounded-md bg-card/90 border border-border/60 px-2 py-0.5 font-mono text-[10px] font-medium text-muted-foreground shadow-sm lg:block">⌘K</kbd>
        </button>
      </div>
      ...
      <div className="flex items-center gap-2 pr-6">
        ...
        <UserMenu profile={profile} userEmail={userEmail} loggingOut={loggingOut} onSignOut={onSignOut} className="hidden h-9 w-9 sm:flex" />
        <UserMenu profile={profile} userEmail={userEmail} loggingOut={loggingOut} onSignOut={onSignOut} className="flex sm:hidden" />
      </div>
    </header>
  );
}
