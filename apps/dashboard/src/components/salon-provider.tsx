...
export function SalonProvider({ children }: SalonProviderProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // iframe can emit auth events (TOKEN_REFRESHED, INITIAL_SESSION, etc.). A foreground reload
      // sets status to "loading", replaces the tree with LoadingScreen, and drops client state
      // (e.g. branding "Show preview"). Only the initial effect load above may show LoadingScreen.
