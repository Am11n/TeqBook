      ...
      if (notificationJob?.id) {
        await admin
          ...
          .update({
            ...
            dead_letter_reason: shouldDeadLetter ? errorReason : null,
            next_retry_at: shouldDeadLetter ? null : getNextRetryAtIso(nextAttempt),
            provider_used: smsResult?.error ? "twilio" : "resend",
          })
          .eq("id", notificationJob.id);
      }
    }
    ...
    if (notificationJob?.id) {
      const attemptRows = [
        ...
        },
        {
          ...
          result: smsResult?.sent ? "success" : "failed",
          error_message: smsResult?.sent ? null : smsResult?.error ?? null,
        },
        {
          notification_job_id: notificationJob.id,
          attempt_no: nextAttempt,
      ...
      await admin.from("notification_attempts").insert(attemptRows);
    }
