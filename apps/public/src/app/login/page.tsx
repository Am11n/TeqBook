        ...
        <div className="relative grid gap-10 p-6 md:p-10 lg:grid-cols-[1.1fr_1.1fr] lg:p-12">
          <div className="flex flex-col justify-center">
            <Link href="/" className="flex items-center gap-3 mb-8 hover:opacity-80 transition-opacity">
              <Image src="/Favikon.svg" alt="TeqBook logo" width={40} height={40} className="drop-shadow-[0_2px_8px_rgba(15,23,42,0.15)]" />
            ...
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-semibold leading-tight text-slate-900">
              {t.welcomeBackTitle.includes("TeqBook") ? (
                <>{t.welcomeBackTitle.split("TeqBook")[0]}<span className="text-blue-700">TeqBook</span>{t.welcomeBackTitle.split("TeqBook")[1] || ""}</>
              ) : (
                <>{t.welcomeBackTitle.split(" ").slice(0, -1).join(" ")} <span className="text-blue-700">TeqBook</span></>
              )}
            </h1>
            <p className="mt-4 max-w-xl text-sm sm:text-base text-slate-600">{t.welcomeBackDescription}</p>
            <ul className="mt-6 space-y-2 text-sm text-slate-700">
              <li className="flex items-start gap-2"><span className="mt-[3px] h-1.5 w-1.5 rounded-full bg-blue-600" /><span>{t.bullet1}</span></li>
              <li className="flex items-start gap-2"><span className="mt-[3px] h-1.5 w-1.5 rounded-full bg-blue-600" /><span>{t.bullet2}</span></li>
              <li className="flex items-start gap-2"><span className="mt-[3px] h-1.5 w-1.5 rounded-full bg-blue-600" /><span>{t.bullet3}</span></li>
            </ul>
            <p className="mt-8 text-xs text-slate-500">{t.trustLine}</p>
          </div>

          <div className="flex items-center justify-center">
            <MotionDiv initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: "easeOut" }} className="w-full max-w-md rounded-3xl bg-white/90 p-6 shadow-[0_22px_60px_rgba(15,23,42,0.18)] backdrop-blur-sm border border-slate-100">
              <div className="mb-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <input id="email" type="email" autoComplete="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder={t.emailPlaceholder} className="w-full rounded-xl border border-slate-200/60 bg-blue-50/80 backdrop-blur-md px-3.5 py-2.5 text-sm text-slate-900 outline-none ring-0 transition focus:border-blue-600 focus:bg-white/90 focus:ring-2 focus:ring-blue-600/30" />
                </div>
                <div className="space-y-1.5">
                  <label htmlFor="password" className="text-sm font-medium text-slate-800">{t.passwordLabel}</label>
