...
- [kode/db+edge+dashboard] Add-on billing: `addon_billing_sync_state` + `addon_billing_sync_snapshot`, `salon_billing_addon_projection`, delt `ensureStripeAddonQuantitiesMatchDb` (`_shared/billing-addon-sync.ts`, `create_prorations`, retry/backoff), `billing-sync-addon-usage` + `billing-reconcile-salons` reconcile-steg, ny `billing-preview-upcoming-invoice` (Stripe `invoices.retrieveUpcoming` kun når `synced`), rate limits + `supabase/config.toml`. Dashboard: debouncet `syncUsageDerivedAddons`, umiddelbar sync på faktureringsside, Stripe-preview som primær estimat, degradert UI, blokkering av plan/payment ved drift/feil/syncing, Starter-tak på tilleggskvanta (synk med edge caps). Migrasjon: `20260502120000_addon_billing_sync_state.sql`.
...
- [kode/db+public] Replay-beskyttelse for `cancel` action-token: ny tabell `public_booking_used_action_token_nonces`, konsum av nonce i `POST /api/bookings/send-cancellation` med `403` ved gjenbruk. Migrasjon: `20260430153000_public_booking_used_action_token_nonces.sql`; `db:manifest:lock` + `supabase db push` kjørt.
...
- [docs+e2e+ci] Phase-2 hardening: OTP cooldown/lockout UX-kontrakt i confirmation flow, migration-integrity CI artifacts (`migration-integrity-evidence`), konkrete SLO terskler i observability/runbook/governance, public booking `@critical` sideeffekt-test (booking -> confirmation), og `E2E journey safety checks` inkluderer nå `tests/e2e/public-booking.spec.ts`.
