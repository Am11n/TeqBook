-- =====================================================
-- Fix: shifts.weekday convention mismatch in get_schedule_segments
-- =====================================================
-- shifts.weekday stores JS getDay() values: 0=Sunday, 1=Monday, ..., 6=Saturday
-- opening_hours.day_of_week stores: 0=Monday, 1=Tuesday, ..., 6=Sunday
--
-- The original RPC converted PostgreSQL DOW (0=Sun) to OH convention (0=Mon)
-- via v_dow_oh, then used v_dow_oh to query BOTH tables. This is wrong for
-- shifts, which use the JS/PG convention. The fix uses v_dow_pg (unchanged
-- PostgreSQL DOW, same as JS getDay) for shifts queries.
-- =====================================================
...
COMMENT ON FUNCTION get_schedule_segments IS
  'Returns all schedule segments for a date: working windows, breaks, time_blocks, bookings, buffers, closed. '
  'Backend is source of truth. FIX: uses v_dow_pg (0=Sunday, JS getDay convention) for shifts.weekday queries.';
