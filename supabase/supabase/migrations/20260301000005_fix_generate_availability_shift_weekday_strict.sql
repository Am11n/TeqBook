-- =====================================================
-- Fix false Sunday availability from compatibility mode
-- =====================================================
-- Root cause:
-- The previous compatibility patch matched shifts.weekday IN (v_dow_pg, v_dow_oh).
-- For example on Sunday:
--   v_dow_pg = 0 (Sun), v_dow_oh = 6 (Sun in opening_hours convention)
-- This can accidentally include Saturday shifts (weekday=6 in JS convention),
-- causing employees to appear available on Sundays when they are off.
--
-- Correct rule (project-wide):
-- - shifts.weekday uses JS/PostgreSQL DOW: 0=Sunday..6=Saturday
-- - opening_hours/day_of_week uses 0=Monday..6=Sunday
--
-- This migration restores strict matching for shifts (v_dow_pg only).
-- =====================================================
...
COMMENT ON FUNCTION generate_availability(UUID, UUID, UUID, DATE)
  IS 'Generates available slots. Strict shifts weekday mapping (0=Sun..6=Sat) to avoid false cross-day availability.';
