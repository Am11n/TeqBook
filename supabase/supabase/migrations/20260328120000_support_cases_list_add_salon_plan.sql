-- =====================================================
-- Support Inbox: expose salon subscription plan + filter
-- =====================================================
...
BEGIN
  ...
  plan_filter := NULLIF(trim(filters->>'salon_plan'), '');
  ...
  WHERE (filters->>'type' IS NULL OR sc.type = filters->>'type')
    ...
    AND (
      plan_filter IS NULL
      OR (s.plan IS NOT NULL AND s.plan::TEXT = plan_filter)
    )
    AND (
      q IS NULL
      OR sc.title ILIKE '%' || q || '%'
...
COMMENT ON FUNCTION get_support_cases_list(JSONB, INTEGER, INTEGER) IS
  'Paginated support cases for Support Inbox. Optional filters.salon_plan = starter|pro|business. Superadmin only.';
