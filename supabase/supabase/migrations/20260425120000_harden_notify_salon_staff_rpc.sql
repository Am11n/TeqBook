-- Harden notify_salon_staff_* RPCs: require booking↔salon consistency and caller authorization
-- (service_role for server paths, or salon staff profile for dashboard JWT).
...
COMMENT ON FUNCTION notify_salon_staff_new_booking(UUID, TEXT, TEXT, TIMESTAMPTZ, UUID, TEXT) IS
  'Notify salon owners/managers about a new booking. Requires booking id to belong to p_salon_id; caller must be service_role or staff profile for that salon.';

COMMENT ON FUNCTION notify_salon_staff_booking_cancelled(UUID, TEXT, TEXT, TIMESTAMPTZ, UUID, TEXT) IS
  'Notify salon owners/managers about a cancelled booking. Same authorization and booking/salon binding as new_booking.';
